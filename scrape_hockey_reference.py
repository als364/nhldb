...
def main():
  print(f"{utils.stringy_now()}: Started scraping", flush=True)
  start = time.time()
  ...
  for year in years:
    ...
    for abbr, team in active_teams_by_abbr.items():
      print(f"{utils.stringy_now()}: Scraping {team.stint_by_year(year).abbr} games from {year}", flush=True)
      ...
      if os.path.isfile(regular_season_filename):
        ...
        print(f"{utils.stringy_now()}: Scraping {data_by_year['abbr']} games from {year}", flush=True)
...
def scrape(urls):
    ...
    for url in urls:
      if url not in broken_urls():
        print(f"{utils.stringy_now()}: Scraping {url}", flush=True)
